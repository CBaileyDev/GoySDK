...
// Callback type for GUI console integration
using ConsoleGuiCallback = std::function<void(const std::string& text, TextColors color)>;
...
public:
	...
	// Register a callback for GUI console integration
	void SetGuiCallback(ConsoleGuiCallback callback) { guiCallback_ = std::move(callback); }

private:
	ConsoleGuiCallback guiCallback_;
};
