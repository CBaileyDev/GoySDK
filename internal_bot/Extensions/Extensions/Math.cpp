...
Vector2DF Vector2DF::Lerp(const Vector2DF& other, float percentage) const
{
	...
	lerpedValue.X = lerp(X, other.X, percentage);
	lerpedValue.Y = lerp(Y, other.Y, percentage);
	return lerpedValue;
}
...
{
	Vector2DF result;
	result.X = midpoint(X, other.X);
	result.Y = midpoint(Y, other.Y);
	return result;
}
...
Vector2DI Vector2DI::Lerp(const Vector2DF& other, int32_t percentage) const
{
	...
	lerpedValue.X = static_cast<int32_t>(lerp(static_cast<float>(X), other.X, static_cast<float>(percentage)));
	lerpedValue.Y = static_cast<int32_t>(lerp(static_cast<float>(Y), other.Y, static_cast<float>(percentage)));
	return lerpedValue;
}
...
VectorF VectorF::Lerp(const VectorF& other, float percentage) const
{
	...
	lerpedValue.X = lerp(X, other.X, percentage);
	lerpedValue.Y = lerp(Y, other.Y, percentage);
	lerpedValue.Z = lerp(Z, other.Z, percentage);
	return lerpedValue;
}
...
{
	VectorI lerpedValue;
	lerpedValue.X = static_cast<int32_t>(lerp<float>(static_cast<float>(X), static_cast<float>(other.X), static_cast<float>(percentage) / 100.0f));
	lerpedValue.Y = static_cast<int32_t>(lerp<float>(static_cast<float>(Y), static_cast<float>(other.Y), static_cast<float>(percentage) / 100.0f));
	lerpedValue.Z = static_cast<int32_t>(lerp<float>(static_cast<float>(Z), static_cast<float>(other.Z), static_cast<float>(percentage) / 100.0f));
	return lerpedValue;
}
