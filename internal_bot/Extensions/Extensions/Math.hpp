...
namespace cpp20_compat {
    ...
    // lerp with mixed types (e.g., int, float, float)
    template<typename T, typename U>
    constexpr auto lerp(T a, T b, U t) noexcept -> decltype(a + t * (b - a)) {
        return a + t * (b - a);
    }
    
    // midpoint with same type
    template<typename T>
...
// Use std if available (C++20), otherwise use our implementation
#if __cplusplus >= 202002L
    using std::lerp;
    using std::midpoint;
#else
    using cpp20_compat::lerp;
    using cpp20_compat::midpoint;
#endif
